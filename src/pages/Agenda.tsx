...
const ConferenceAgenda = () => {
  ...
  const fetchAgenda = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const parsedAgenda = JSON.parse(data.agendaData); // agendaData is a JSON string
        setAgenda(parsedAgenda);

        // set default active day to the first one
        if (Object.keys(parsedAgenda).length > 0) {
  ...
  return (
    <>
      ...
      <div className="bg-[#E6F4F1] min-h-screen py-12 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-4xl font-bold text-nursing-primary mb-2">
            Conference Agenda
          </h1>
          <p className="text-lg text-gray-700 mb-10">
            Explore our comprehensive program of events and sessions
          ...
          {loading ? (
            ...
            <p className="text-gray-600">Agenda will be updated soon.</p>
          ) : (
            <>
              {/* Day Selector */}
              <div className="inline-flex space-x-2 mb-6">
              ...
              {activeDay && (
                <Card className="shadow-lg p-6 bg-white text-left">
                  <h2 className="text-lg font-semibold text-nursing-primary mb-4">
                    {activeDay}: {agenda[activeDay]?.date}
                  </h2>
                  {agenda[activeDay]?.sessions?.length === 0 ? (
                    <p className="text-gray-600 text-sm">
                      Agenda will be updated soon.
                    </p>
                  ) : (
                    <ul className="divide-y divide-gray-200">
                      {agenda[activeDay]?.sessions?.map((session, idx) => (
                        <li
                          key={idx}
                          className="py-4 flex flex-col md:flex-row md:justify-between"
                        >
              ...
              <div className="mt-6">
                <Button variant="default">Download Full Agenda</Button>
                <p className="mt-2 text-xs text-gray-500">
                  * The agenda is subject to change. More sessions and speakers
