...
const ContactPage = () => {
  ...
  const [loading, setLoading] = useState(false);
  const [venue, setVenue] = useState<string>("Loading venue...");
  ...
  useEffect(() => {
    const fetchVenue = async () => {
      try {
        ...
        setVenue("Venue details will be updated soon.");
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`https://nursing.marketingzynlogic.com/api/nursing/contact-form`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const text = await res.text(); // response is plain text
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-8 px-6 py-16">
          ...
          <div className="bg-white shadow-md rounded-lg p-8">
            ...
            <p>
              <span className="font-medium text-gray-700">Phone:</span>
              <br />
              <a href="tel:+17163749009" className="text-green-600 font-medium">
                📞 +1 (716) 374-9009
              </a>
            </p>
          </div>
