...
import {
  AlignJustify, Search, Bell, Settings, Rss, Calendar, MessageCircle, File, Mail, List, Layout, FormInput,
    ...
    <div className="flex min-h-screen bg-gray-100 font-sans">
      ...
      <aside className={`fixed z-50 h-full bg-[#0A234E] text-white transition-all duration-300 ease-in-out overflow-y-auto ${isSidebarOpen ? 'w-64 translate-x-0' : '-translate-x-full md:w-64 md:translate-x-0'}`}>
        ...
        <nav className="p-4 space-y-2">
          ...
          <SidebarItem icon={Home} label="E-Commerce" isActive={location.pathname === '/admin'} onClick={() => navigate('')} />
      ...
      <div className={`flex-1 transition-all duration-300 ease-in-out ${isSidebarOpen ? 'md:ml-64' : 'md:ml-0'}`}>
        ...
        <header className="sticky top-0 z-40 bg-white shadow-sm border-b border-gray-200 p-4 flex justify-between items-center transition-all duration-300 ease-in-out">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-2xl font-bold text-gray-900">NURSING MEET 2026</h1>
          </div>
          <div className="flex items-center space-x-4">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search"
                className="pl-10 pr-4 py-2 rounded-full bg-gray-100 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200 w-40 sm:w-64"
              />
            </div>
            <Bell className="text-gray-600 hover:text-gray-900 cursor-pointer w-6 h-6" />
            ...
            <div className="relative w-8 h-8 rounded-full overflow-hidden border-2 border-gray-300 cursor-pointer">
              <img src="https://placehold.co/100x100/A0B9C6/FFFFFF?text=P" alt="Profile" className="object-cover w-full h-full" />
            </div>
          </div>
        </header>
