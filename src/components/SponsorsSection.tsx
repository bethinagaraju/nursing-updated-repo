...
//       {
//         time: "09:00 - 10:00",
//         title: "Keynote: Digital Health Transformationsss",
...
const ConferenceAgenda = () => {
  // State to hold the agenda data from the API
  ...
  useEffect(() => {
    ...
    const fetchAgenda = async () => {
      try {
        const response = await fetch("https://nursing.marketingzynlogic.com/api/nursing-agenda");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        // The API returns a stringified JSON, so we need to parse it
        const parsedAgenda = JSON.parse(data.agendaData);
        setAgenda(parsedAgenda);
        // Set the first day as the default active day
        setActiveDay(Object.keys(parsedAgenda)[0] || "");
      } catch (e) {
        setError(e.message);
  ...
  // Safely get the details for the active day, providing defaults
  const { date, sessions } = agenda[activeDay] || { date: "", sessions: [] };
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <p className="text-lg">Loading Agenda...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <p className="text-lg text-red-500">Error fetching agenda: {error}</p>
      </div>
    );
  }
      ...
      <div className="bg-[#E6F4F1] min-h-screen py-12 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <Card className="shadow-lg p-6 bg-white text-left">
            ...
            {sessions.length === 0 ? (
              <p className="text-gray-600 text-sm">Agenda for this day will be updated soon.</p>
            ) : (
              <ul className="divide-y divide-gray-200">
                {sessions.map((session, idx) => (
                  <li
